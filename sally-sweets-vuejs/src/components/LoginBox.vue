<template>
  ...
  <form class="max-w-sm mx-auto my-8" @submit.prevent="signInWithEmailAndPassword">
    ...
    <input type="email" v-model="email" id="email" class="mt-1 p-2 w-full border rounded-md bg-red-100">
  </div>
  <div class="mb-4">
    <label for="password" class="block text-sm font-medium text-gray-600">Password</label>
          <input type="password" v-model="password" id="password" class="mt-1 p-2 w-full border rounded-md bg-red-100">
        </div>

  <button class="mt-4 text-black px-4 py-2 rounded-md focus:outline-none focus:ring transition duration-300 hover:bg-rose-900"
...
<button class="flex items-center mt-4 text-black px-4 py-2 rounded-md focus:outline-none focus:ring transition duration-300 hover:bg-rose-900"
...
<br>
<div>
<router-link to="/register" class="text-gray-500 focus:outline-none hover:text-blue-600 brounded-lg text-sm p-2.5">
      New to Sally Sweets?
    </router-link>
  </div>
</form>
